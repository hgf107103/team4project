 ...
 */
@WebServlet("/master/user/out")
public class masterUserOutServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;
    public masterUserOutServlet() {
        super();
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
				...
				if (userCheck != null) {//로그인된 유저가 어드민일 경우 실행
					...
					if (selectUserTemp != null) {
						...
						if(selectUserTemp.getUserOutCheck() == 0) {
							int updateCheck = sqlse.update("userMapper.updateUserOutDay", userNumber);
			...
			System.out.println("masterUserOutServlet ERROR : " + e);
			sqlse.close();
			response.sendError(400);
		}
	}
